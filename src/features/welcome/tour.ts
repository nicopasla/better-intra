...
const TOUR_HOST_ID = "welcome-tour-host";
...
async function collectSteps(): Promise<TourStep[]> {
  const present = () => TOUR_STEPS.filter((s) => resolveTarget(s));
...
export async function startTour(): Promise<void> {
  document.getElementById(TOUR_HOST_ID)?.remove();
  ...
  const scrim = (p: string) =>
    root.querySelector<HTMLElement>(`.bi-tour-scrim[data-p="${p}"]`)!;
  const scrims = {
    top: scrim("top"),
    bottom: scrim("bottom"),
    left: scrim("left"),
  ...
  const ring = root.querySelector<HTMLElement>(".bi-tour-ring")!;
  const tip = root.querySelector<HTMLElement>(".bi-tour-tip")!;
  ...
  function refreshTarget(): boolean {
    target = resolveTarget(steps[index]);
